           ...
           (delq nil
                 (list
                  ...
                  (and (boundp 'byte-compile-current-file)
                       ...
                       (file-name-directory
                        (expand-file-name byte-compile-current-file)))
                  (let ((lib (locate-library "ffi")))
         ...
         (module-path
          (catch 'found
            (dolist (dir candidate-dirs)
              (let ((candidate (expand-file-name (concat "ffi-module" ext) dir)))
                (when (file-exists-p candidate)
                  (throw 'found candidate)))))))
    (if module-path
        (module-load module-path)
      (if (bound-and-true-p byte-compile-current-file)
          (message "ffi.el (compilation): Skipping module load, binary not found")
        (error "ffi.el: Could not find ffi-module%s near %s"
               ext
               (mapconcat #'identity candidate-dirs ", "))))))
